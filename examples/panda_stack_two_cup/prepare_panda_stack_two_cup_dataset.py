#!/usr/bin/env python3
"""Prepare panda_stack_two_cup_ee_fullres for GR00T N1.7 (NEW_EMBODIMENT).
...
The source dataset ships as LeRobot v3.0 on the Hub (repo id suffix
"_v3.0") and must be converted to v2.1 first via
scripts/lerobot_conversion/convert_v3_to_v2.py (its own sub-venv -- see
that script's docstring) before this script can run against it; this
script itself only understands the v2.1 per-episode-parquet layout, same
as the panda_drawer script.
...
dataset (unlike panda_drawer's, which was checked against imikit's
actual recording code). If any column names differ, adjust
process_episode() below and the "video" section of write_modality_json()
to match before running.

Usage:
    python prepare_panda_stack_two_cup_dataset.py --dataset-path <path>
"""
